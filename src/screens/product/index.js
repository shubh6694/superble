...
//https://api-dev.superble.com/api/v1/products/1732/questions
    ...
    commentLaps = (commentDataNew = null) => { 
    	...
    	if(commentData !== null){
		    ...
		    // alert(articleData.question_text);
		    	...
		    	count++;
		    	let owner_image = 'https://forums.iboats.com/user/avatar?userid=503684&type=large';
		    	if(articleData.user.image_url != null){
		    		owner_image = articleData.user.image_url;
		    	}
                return (
                  <View key={articleData.id} style={styles.imageCommentWrap}>
							...
							<View style={styles.likeImgContainer}>
								...
								{ this.state.isLikeComment == false &&
									<TouchableHighlight onPress={() => this.likeComment(this.state.productId,articleData.id)}>
									<Image 
    ...
    likeComment = (productId,questionId) =>{
		...
		.then( (value) => {
				if(value === null) {
			      ...
			      fetch("https://api-staging.superble.com/api/v1/superbles/superble_question", {
			          method: 'POST',
			          headers: {
	...
	render() {
		...
		if(record !== null){
		    ...
		    if(record.product.owner_image != null){
				owner_image = record.product.owner_image;
			}
		}
		if(record != null){
										...
										<ScrollView	> 
											{ record.product.description != '' && 
 											<View style={styles.imageCommentWrap}>						                  		
													...
													<View style={styles.contentContainer}>
													{record.product.product_owner != null && 	
														<Text style={{lineHeight:0}}>
															<H1 style={[styles.text, styles.name]}>{record.product.product_owner}</H1>
														</Text>
													}
													{record.product.description != null && 
														<Text style={styles.commentText}>{record.product.description}</Text> 
													}
													</View>
													<View style={styles.likeImgContainer}>
